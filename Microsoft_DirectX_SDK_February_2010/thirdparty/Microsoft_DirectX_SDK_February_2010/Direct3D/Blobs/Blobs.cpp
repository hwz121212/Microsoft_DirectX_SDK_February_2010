...
HRESULT FillBlobVB( D3DXMATRIXA16* pmatWorldView )
{
    ...
    for( i = 0; i < NUM_BLOBS; ++i )
    {
        ...
        for( int j = 0; j < 6; ++j )
        {
            ...
            pBlobVertex[posCount].tBack = D3DXVECTOR2( ( 0.5f + pBlobVertex[posCount].pos.x ) *
                                                       ( 1.0f / pBackBufferSurfaceDesc->Width ),
                                                       ( 0.5f + pBlobVertex[posCount].pos.y ) *
                                                       ( 1.0f / pBackBufferSurfaceDesc->Height ) );
...
HRESULT RenderFullScreenQuad( float fDepth )
{
    ...
    aVertices[3].pos = D3DXVECTOR4( pBackBufferSurfaceDesc->Width - 0.5f, pBackBufferSurfaceDesc->Height - 0.5f,
                                    fDepth, fDepth );
    ...
    aVertices[0].vColor = aVertices[1].vColor = aVertices[2].vColor = aVertices[3].vColor = D3DXVECTOR3(0,0,0);
...
void CALLBACK KeyboardProc( UINT nChar, bool bKeyDown, bool bAltDown, void* pUserContext )
{
    if( bKeyDown )
    {
        switch( nChar )
        {
            ...
            case VK_F2:
                if( DXUTIsTimePaused() ) DXUTPause( false, false );
        }
    }
}
