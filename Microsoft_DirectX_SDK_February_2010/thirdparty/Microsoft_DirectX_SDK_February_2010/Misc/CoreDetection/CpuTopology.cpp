...
private:
    ...
    // However, the __cpuid intrinsic does not enable the ability to set the ECX register,
    // which is required for obtaining certain extended CPU information. For VS 2008 or
    // later we can use __cpuidex instead.

    // For VS 2005, we must call the Cpuid64() external function, which is written in
    // assembly and located in the cpuid64.asm file included in this project.  This
    // project contains a build step that invokes 64-bit MASM on cpuid64.asm when
    // building a 64-bit target.
