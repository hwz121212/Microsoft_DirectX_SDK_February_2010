...
// Textures
//--------------------------------------------------------------------------------------
Texture2D g_DensityTexture :     register( t2 );  // Height map density
...
struct VS_OUTPUT_NO_TESSELLATION
{
    ...
    float3 vLightTS  : LIGHTVECTORTS;   // light vector in tangent space, denormalized

#if ADD_SPECULAR==1
    float3 vViewTS   : VIEWVECTORTS;    // view vector in tangent space, denormalized
#endif
...
#if TEXTURE_COORDINATES_CALCULATION_FROM_MODEL_OPTIMIZATION==0
    float2 texCoord  : TEXCOORD0;
#endif
...
VS_OUTPUT_NO_TESSELLATION VS_NoTessellation( VS_INPUT i )
{
    ...
    // Transform normal, tangent and binormal vectors from object space to homogeneous projection space and normalize them
	...
	float3 vNormalTS = normalize(vNormalHeight.xyz * 2 - 1);
...
VS_OUTPUT_HS_INPUT VS( VS_INPUT i )
{
	...
	const float fMinTessellationFactor = 3.0f;	// Min tessellation factor to apply regardless of distance
	...
	Out.fVertexDistanceFactor = 1.0 - clamp( ((fDistance - fMinDistance) / (fMaxDistance - fMinDistance)), 0.0, 1.0 - fMinTessellationFactor/g_vTessellationFactor.x);
#endif
...
HS_CONSTANT_DATA_OUTPUT ConstantsHS( InputPatch<VS_OUTPUT_HS_INPUT, 3> p, uint PatchID : SV_PrimitiveID )
{
	...
	// Calculate edge scale factor from vertex scale factor: simply compute average tess factor between the two vertices making up an edge
	float3 fScaleFactor;
...
DS_OUTPUT DS( HS_CONSTANT_DATA_OUTPUT input, float3 BarycentricCoordinates : SV_DomainLocation, const OutputPatch<HS_CONTROL_POINT_OUTPUT, 3> TrianglePatch )
{
	...
	output.vViewTS  = BarycentricCoordinates.x * TrianglePatch[0].vViewTS  + BarycentricCoordinates.y * TrianglePatch[1].vViewTS  + BarycentricCoordinates.z * TrianglePatch[2].vViewTS;
#endif
	...
	output.vDiffuseColor = BarycentricCoordinates.x*input.VertexDensity[0] + BarycentricCoordinates.y*input.VertexDensity[1] + BarycentricCoordinates.z*input.VertexDensity[2];
#endif
