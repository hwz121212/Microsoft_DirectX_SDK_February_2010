...
//
// Demonstrates how to use Compute Shader perform sorting on the GPU with DirectX 11.
//
...
// At minimum 8,096 elements - BITONIC_BLOCK_SIZE * TRANSPOSE_BLOCK_SIZE
...
void SetConstants( UINT iLevel, UINT iLevelMask, UINT iWidth, UINT iHeight )
{
    ...
    g_pd3dImmediateContext->Map( g_pCB, 0, D3D11_MAP_WRITE_DISCARD, 0, &MappedResource );
    memcpy( MappedResource.pData, &cb, sizeof(cb) );
    g_pd3dImmediateContext->Unmap( g_pCB, 0 );
...
void GPUSort()
{
    ...
    for( UINT level = 2 ; level <= BITONIC_BLOCK_SIZE ; level <<= 1 )
    {
        ...
        g_pd3dImmediateContext->CSSetUnorderedAccessViews( 0, 1, &g_pBuffer1UAV, &UAVInitialCounts );
    ...
    for( UINT level = (BITONIC_BLOCK_SIZE << 1) ; level <= NUM_ELEMENTS ; level <<= 1 )
    {
        ...
        g_pd3dImmediateContext->CSSetUnorderedAccessViews( 0, 1, &g_pBuffer2UAV, &UAVInitialCounts );
