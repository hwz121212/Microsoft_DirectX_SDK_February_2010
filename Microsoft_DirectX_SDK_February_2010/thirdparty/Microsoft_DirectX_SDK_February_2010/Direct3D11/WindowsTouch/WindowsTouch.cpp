...
// Multi-touch requires Windows 7 and the Windows 7 RC SDK (or later).
#include "sdkddkver.h"
...
HRESULT CALLBACK OnD3D11CreateDevice( ID3D11Device* pd3dDevice, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext  )
{
    ...
    if ( g_FeatureLevel > D3D_FEATURE_LEVEL_9_3 ) {
        V_RETURN( CompileShaderFromFile( L"Soldier.fx", "VSSkinnedmain", pszVSModel, &VSSkinnedmain ) );
    ...
    V_RETURN( CompileShaderFromFile( L"Soldier.fx", "VSmain", pszVSModel, &VSmain ) );
    V_RETURN( pd3dDevice->CreateVertexShader( VSmain->GetBufferPointer(),
        VSmain->GetBufferSize(), NULL, &g_pRenderStillVS) );

    V_RETURN( CompileShaderFromFile( L"Soldier.fx", "PSSkinnedmain", pszPSModel, &PSSkinnedmain ) );
    ...
    if ( g_FeatureLevel > D3D_FEATURE_LEVEL_9_3 ) {
        V_RETURN( CompileShaderFromFile( L"quad.hlsl", "VSMain", pszVSModel, &QuadVSBlob ) );
        V_RETURN( pd3dDevice->CreateVertexShader( QuadVSBlob->GetBufferPointer(),
            QuadVSBlob->GetBufferSize(), NULL, &g_pQuadVS) );

        V_RETURN( CompileShaderFromFile( L"quad.hlsl", "PSMain", pszPSModel, &QuadPSBlob ) );
